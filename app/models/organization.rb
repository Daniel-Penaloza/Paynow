class Organization < ApplicationRecord
  # MONETIZATION DISABLED — descomentar cuando se quiera activar cobros
  # PLAN_LIMITS = {
  ...
  def on_trial?    = false
  def plan_active? = true

  # Sin límites mientras la monetización está deshabilitada
  def within_business_limit? = true
  def within_receipt_limit?  = true

  # MONETIZATION DISABLED — lógica original:
  # def on_trial?
  ...
  #   monthly_count = businesses.joins(:receipts).where(receipts: { created_at: start_of_month.. }).count
  #   monthly_count < limit
  # end
